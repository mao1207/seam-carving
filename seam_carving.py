...
class SeamCarver:
    ...
    def cumulative_map_forward(self, energy_map):
        ...
        calculative_map = np.copy(energy_map)
        for row in range(1, height):
            for col in range(width):
                if col == 0:
                    e_right = calculative_map[row - 1, col + 1] + matrix_x[row - 1, col + 1] + matrix_y_right[row - 1, col + 1]
                    e_up = calculative_map[row - 1, col] + matrix_x[row - 1, col]
                    calculative_map[row, col] = energy_map[row, col] + min(e_right, e_up)
                elif col == width - 1:
                    e_left = calculative_map[row - 1, col - 1] + matrix_x[row - 1, col - 1] + matrix_y_left[row - 1, col - 1]
                    e_up = calculative_map[row - 1, col] + matrix_x[row - 1, col]
                    calculative_map[row, col] = energy_map[row, col] + min(e_left, e_up)
                else:
                    e_left = calculative_map[row - 1, col - 1] + matrix_x[row - 1, col - 1] + matrix_y_left[row - 1, col - 1]
                    e_right = calculative_map[row - 1, col + 1] + matrix_x[row - 1, col + 1] + matrix_y_right[row - 1, col + 1]
                    e_up = calculative_map[row - 1, col] + matrix_x[row - 1, col]
                    calculative_map[row, col] = energy_map[row, col] + min(e_left, e_right, e_up)
        return calculative_map
